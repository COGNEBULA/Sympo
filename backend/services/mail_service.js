...
async function sendWelcomeMail(receipt) {
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: "🎉 Welcome to COGNEBULA 26",
    html: `
<!DOCTYPE html>
<html>
<head>
  ...
  <div class="wrapper">
    <div class="card">
      ...
      <div class="hero">
        ...
        <div class="badge">WELCOME TO VEC</div>
        ${receipt_id
...
async function checkInMail(receipt) {
  const {
    receipt_id,
    participant,
    events,
  ...
  const { name, email, second_email } = participant;
  const receivers = second_email
    ? `${second_email}`
    : email;
...
module.exports = { sendWelcomeMail, checkInMail };
